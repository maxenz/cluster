...
class CreateEditRequestForm extends Component {
  ...
  handleFileChange = (e, results) => {
    ...
    if (file.name.toUpperCase().endsWith("STL")) {
      this.setState({
        uploadedFile: file,
        fileName: `${moment()}_${file.name}`
      });
      this.clearError();
    } else {
      this.setState({
        error: { message: "Debe subir un archivo con formato .stl" }
      });
    }
  };
  ...
  onSubmit = () => {
    ...
    const apikey = "ArCGBdA2yRP6sNrIf24e5z";
  ...
  formIsInvalid = () => {
    const { uploadedFile, colorType, materialType, diameter } = this.state;
          ...
          >
            ...
            >
              <Dropdown
                ...
                onChange={(event, data) =>
                  this.handleChange("materialType", data.value)
                }
              />
              ...
              <Dropdown
                ...
                options={colorsList}
                onChange={(event, data) =>
                  this.handleChange("colorType", data.value)
                }
              />
              ...
              <Form.Input
                ...
                value={diameter}
                onChange={event =>
                  this.handleChange("diameter", event.target.value)
                }
              />
              ...
              <Button
                ...
                fluid
                size="large"
                disabled={this.formIsInvalid() || isSaving}
              >
                Generar
              </Button>
