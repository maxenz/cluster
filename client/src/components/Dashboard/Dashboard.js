...
export class Dashboard extends React.Component {
  ...
  getPrintersInfo = () => {
    ...
    printers.forEach(printer => {
      const reqMade = requests.filter(
        r =>
          r.selected_printer_id === printer._id &&
          (r.status === REQUESTS_STATUS_READY_TO_DELIVER ||
            r.status === REQUESTS_STATUS_DONE)
      ).length;
      const actualRequest = requests.filter(
        r =>
          r.status === REQUESTS_STATUS_PRINTING &&
          r.selected_printer_id === printer._id
      );
      const pendingMinutes =
        actualRequest.length > 0
          ? moment(actualRequest[0].finish_printing_time).diff(
              moment(),
              "minutes",
              true
            )
  ...
  getSemanalUse = () => {
    const requests = Object.values(this.props.requests);
    const semanalRequests = requests.filter(
      r => moment().diff(moment(r.start_printing_time), "days", true) < 7
    );
  ...
  componentWillMount() {
    if (
      ...
      (this.props.isAuthenticated && !this.props.auth.user.admin)
    ) {
